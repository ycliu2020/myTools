...
function [readme, Experiment, level, tLin, mPlev, vars] = cmipParameters(p_1)
    ...
    tLin.endYear = {2014, 2014, 2099, 2099, 2014, 2014}; % start year(note that 4xco2 not have endyear)
    tLin.inter = {15 * 12 35 * 12 85 * 12 85 * 12 15 * 12 35 * 12 150 * 12 15 * 12 35 * 12}; % total months
    % tLin.start = [253 13 1 1 1561 1321];

    %% pleve test
    mPlev.units = {'Pa', 'Pa', 'Pa', 'Pa', 'Pa', 'Pa', 'Pa', 'Pa', 'Pa'};
    mPlev.wards = {'down', 'down', 'down', 'down', 'down', 'down', 'down', 'down', 'down'};
    mPlev.length = {19, 19, 19, 19, 19, 19, 19, 19, 19}; % length(plev)
    mPlev.maxx = {10e4, 10e4, 10e4, 10e4, 10e4, 10e4, 10e4, 10e4, 10e4};
    mPlev.minn = {100, 100, 100, 100, 100, 100, 100, 100, 100};
    %% vars name
    vars.all = {'hus', 'ta', ...
