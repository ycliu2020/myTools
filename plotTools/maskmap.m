...
%  对所绘制图形进行白化
% 例子
% [xc,yc]=maskmap([dir,'\中国边界数据\国界线\bou1_4p.shp'],true,'lon',lon1,'lat',lat1,'linewidth',0.5, 'edgecolor','b');
% [max, may] = m_ll2xy(xc, yc);
% mapshow(max, may, 'displaytype', 'polygon', 'facecolor', 'w');
%该函数输出的是一组坐标包含要白化的坐标
%输出参数：xc ,yc标准坐标。    
%  输入参数：
